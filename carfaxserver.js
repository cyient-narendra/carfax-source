...
const processflow = require('./class');
...
app.post("/dataentry", async (req, res, next) => {
    ...
    try {
        let pool = await mssql.GetCreateIfNotExistPool(sqlConfig);
...
app.post("/hyperfilename", async (req, res) => {
    try {
        let pool1 = await mssql.GetCreateIfNotExistPool(sqlConfig);
...
app.post("/getparties", async (req, res) => {
    try {
        let pool2 = await mssql.GetCreateIfNotExistPool(sqlConfig);
...
app.post("/getvechicles", async (req, res) => {
    try {
        let pool3 = await mssql.GetCreateIfNotExistPool(sqlConfig);
...
app.post("/getpassengers", async (req, res) => {
    ...
    try {
        let pool4 = await mssql.GetCreateIfNotExistPool(sqlConfig);
...
app.post("/hyperEntryData", async (req, res, next) => {
    ...
    try {
        let pool5 = await mssql.GetCreateIfNotExistPool(sqlConfig);
...
app.post("/getname", async (req, res) => {
    try {
        ...
        let qr = "Select Distinct Username from [CarFax_Staging2_Hold].[dbo].[Login] ";
        //console.log(`Query  ---> ${qr}`);
        pool.request().query(qr).then(function (result, err) {
            //console.log("*** Data successfully returned *** ");
            //console.log(result);
...
app.post("/getMetroFiles", async (req, res) => {
    try {
        ...
        if (_step == 'QC') {
            qr = "Select *,CONVERT(varchar,Received_Date,23) as ReceivedDate from [CarFax_Staging].[dbo].[Accident] where agency_ori in ('METRO16DI','METROCFX1') AND (QC_By is null or QC_By = '') AND DC_Status = 'COMP' AND Commerce_Type = '" + _commerce + "' ";
        }
        else if (_step == 'DC') {
            qr = "Select *,CONVERT(varchar,Received_Date,23) as ReceivedDate from [CarFax_Staging].[dbo].[Accident] where agency_ori in ('METRO16DI','METROCFX1') AND (DC_Name is null or DC_Name = '') AND Commerce_Type = '" + _commerce + "' ";
        }
        else if (_step == 'QA') {
            qr = "Select *,CONVERT(varchar,Received_Date,23) as ReceivedDate from [CarFax_Staging].[dbo].[Accident] where agency_ori in ('METRO16DI','METROCFX1') AND (QA_By is null or QA_By = '') AND DC_Status = 'COMP' AND QC_Status = 'COMP' AND Commerce_Type = '" + _commerce + "'";
        }
        else if (_step == 'Rework') {
            qr = "Select *,CONVERT(varchar,Received_Date,23) as ReceivedDate from [CarFax_Staging].[dbo].[Accident] where agency_ori in ('METRO16DI','METROCFX1') AND DC_Status = 'COMP' AND QC_Status = 'REJECT' AND Commerce_Type = '" + _commerce + "'";
        }
        ...
        for (let i = 0; i < _org.length; i++) {
            ...
            pool.request().query(qr).then(function (result, err) {
                ...
                if (result.rowsAffected > 0) {
                    ...
                    return res.json(
                        {
                            "err": "Error In Work Allocation, Please Check"
                        })
                }
            })
        }
...
app.post('/datauploadfile', function (req, res) {
    sql.connect(sqlConfig).then(function (pool, err) {
...
app.post("/getdeliveryFilesCount", async (req, res) => {
    try {
        let pool = await mssql.GetCreateIfNotExistPool(sqlConfigstaging);
...
app.post("/xmlConvert", async (req, res) => {
    try {
        ...
        for (var i = 0; i < _file.length; i++) {
            let recdt = _file[i].Received_Date.toString().split('T')[0].replace(/[-]/g, '_');
            console.log(recdt);
            const file = "http://172.17.30.56:80/carfaxinputs/February_2022/" + _file[i].Metro_Type + '/' + recdt + '/' + _file[i].File_org_name;
            console.log(file);
        ...
        for (var i = 0; i < _file.length; i++) {
            ...
            request.input('PDF_Name', _file[i].pdf_name)

            qr = `Insert into [CarFax_Staging].[dbo].[UIDs_user] (id,PDF_Name,[User]) values('${_file[i].UID}','${_file[i].pdf_name}','${_user}'); `
            console.log(`Query  ---> ${qr}`);
            pool.request().query(qr).then(function (result, err) {
                console.log(result);
                console.log(qr);
...
app.post("/getdcName", async (req, res) => {
    try {
        let pool = await mssql.GetCreateIfNotExistPool(sqlConfig);

        //console.log("use ="+_user);
        let qr = "Select Username,Full_Name,TL from [CarFax_Staging2_Hold].[dbo].[Login] where Exist =1 order by Username;"
        //console.log(`Query  ---> ${qr}`);
        pool.request().query(qr).then(function (result, err) {
            //console.log("*** Data successfully returned *** ");
            //console.log(result);
